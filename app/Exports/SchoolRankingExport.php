...
class SchoolRankingExport implements WithMultipleSheets
{
    ...
    {
        $sheets = [new SchoolRankingAllSheet($this->ranking, $this->title)];

        foreach ($this->ranking->groupBy('class_name') as $className => $rows) {
            $sheets[] = new SchoolRankingClassSheet(
...
class SchoolRankingAllSheet implements FromCollection, WithHeadings, WithMapping, WithTitle
{
    public function __construct(
...
class SchoolRankingClassSheet implements FromCollection, WithHeadings, WithMapping, WithTitle
{
    public function __construct(
        private Collection $rows,

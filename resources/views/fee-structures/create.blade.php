<x-app-layout>
    <x-slot name="title">Add Fee Structure</x-slot>

    <div class="kia-page-header">
        <div><h1 class="kia-page-title">Add Fee Structure</h1></div>
        <a href="{{ route('fee-structures.index') }}" class="btn btn-ghost">← Back</a>
    </div>

    <div class="kia-card" style="max-width:520px">
        <form method="POST" action="{{ route('fee-structures.store') }}" class="kia-form">
            @csrf
            <div class="kia-form-group">
                <label class="kia-label">Fee Name *</label>
                <input name="name" value="{{ old('name') }}" class="kia-input @error('name') is-invalid @enderror" placeholder="e.g. Tuition Fee">
            ...
            <div class="kia-form-row kia-form-row--2">
                <div class="kia-form-group">
                    ...
                    <input name="amount" type="number" step="0.01" min="0.01" value="{{ old('amount') }}" class="kia-input @error('amount') is-invalid @enderror">
                    @error('amount')<span class="kia-field-error">{{ $message }}</span>@enderror
                </div>
                <div class="kia-form-group">
                    <label class="kia-label">Frequency *</label>
                    <select name="frequency" class="kia-select @error('frequency') is-invalid @enderror">
                        @foreach(['once','monthly','term','annual'] as $f)
                            <option value="{{ $f }}" @selected(old('frequency') === $f)>{{ ucfirst($f) }}</option>
                        @endforeach
                    </select>
                    @error('frequency')<span class="kia-field-error">{{ $message }}</span>@enderror
                </div>
            </div>
            <div class="kia-form-group">
                <label class="kia-label">Applies To Class (leave blank = all)</label>

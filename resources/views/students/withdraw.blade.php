<x-app-layout>
    <x-slot name="title">{{ __('student_transfer.withdraw_title') }}</x-slot>

    <div class="kia-page-header">
        <div>
            <h1 class="kia-page-title">{{ __('student_transfer.withdraw_title') }}</h1>
    ...
    <div class="kia-card" style="max-width:640px;">
        <div class="kia-card-body">
            ...
            <form method="POST" action="{{ route('students.withdraw', $student) }}">
                @csrf
                ...
                <button type="submit" class="btn btn-danger" onclick="return confirm('{{ __('student_transfer.withdraw_confirm') }}')">{{ __('student_transfer.withdraw_title') }}</button>
            </form>
        </div>
    </div>
</x-app-layout>

<div class="kia-card" style="margin-top:20px;">
    ...
    <div class="kia-card-body">
        @can('staff.edit')
        <form method="POST" action="{{ route('staff-documents.store', $staff) }}" enctype="multipart/form-data"
            ...
            <div class="form-group" style="flex:1;min-width:160px;margin-bottom:0;">
                <label class="form-label" for="staff-doc-label">{{ __('Label') }}</label>
                <input type="text" id="staff-doc-label" name="label" class="form-control {{ $errors->has('label') ? 'is-invalid' : '' }}"
                       maxlength="150" required placeholder="{{ __('e.g. CV / Resume') }}">
                @error('label')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>
            <div class="form-group" style="flex:1;min-width:200px;margin-bottom:0;">
                <label class="form-label" for="staff-doc-file">{{ __('File') }}</label>
                <input type="file" id="staff-doc-file" name="file" class="form-control {{ $errors->has('file') ? 'is-invalid' : '' }}"
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;gap:14px;padding:10px 0;border-bottom:1px solid var(--line);flex-wrap:wrap;">
            ...
            <div style="display:flex;gap:8px;">
                <a href="{{ route('staff-documents.download', $doc) }}" class="btn btn-sm btn-outline">{{ __('Download') }}</a>
                @can('staff.edit')
                <form method="POST" action="{{ route('staff-documents.destroy', $doc) }}" onsubmit="return confirm('{{ __('Delete this document?') }}')">
                    @csrf @method('DELETE')

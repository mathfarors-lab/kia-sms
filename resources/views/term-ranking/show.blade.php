<x-app-layout>
    ...
    <div class="kia-page-header">
        ...
        <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
            <a href="{{ route('term-ranking.excel', [$academicYear, $semesterSlug]) }}" class="btn btn-ghost">{{ __('academic_ranking.download_excel') }}</a>
            <a href="{{ route('term-ranking.pdf', [$academicYear, $semesterSlug]) }}" class="btn btn-ghost" target="_blank">{{ __('academic_ranking.download_pdf') }}</a>
            <a href="{{ route('term-ranking.index') }}" class="btn btn-ghost">{{ __('academic_ranking.back') }}</a>
        </div>
    </div>
    ...
    <div class="kia-card" style="margin-bottom:16px;">
        <div class="kia-card-body" style="padding:.75rem 1rem;">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                ...
                <a href="{{ route('term-ranking.show', [$academicYear, $semesterSlug]) }}"
                   class="btn btn-xs {{ !$filterClass ? 'btn-primary' : 'btn-outline' }}">{{ __('academic_ranking.all') }}</a>
                @foreach($classes as $cls)
                <a href="{{ route('term-ranking.show', [$academicYear, $semesterSlug, 'class' => $cls]) }}"
        ...
        <div class="kia-table-wrap">
            <table class="kia-table">
                ...
                <tbody>
                    ...
                    @empty
                    <tr><td colspan="9" class="kia-table-empty">{{ __('academic_ranking.no_results_term') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
